  ...
  return (
    <div className="group glass rounded-xl p-4 transition-all hover:ring-1 hover:ring-white/10">
      <div className="flex items-start gap-3">
        <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg border border-border/50 bg-secondary/30 text-lg">
          {typeOpt?.icon ?? '📝'}
        </div>
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95, y: 12 }} animate={{ opacity: 1, scale: 1, y: 0 }} className="glass w-full max-w-md rounded-xl border border-border/50 p-6 shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-3.5">
          ...
          <div>
            ...
            <textarea value={notes} onChange={e => setNotes(e.target.value)} rows={2} placeholder="ما الذي تم التحدث عنه..." className="w-full rounded-lg border border-border/50 bg-secondary/30 px-3 py-2 text-xs text-foreground placeholder:text-muted-foreground/40 resize-none focus:outline-none focus:border-primary/50" />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="mb-1.5 block text-xs text-muted-foreground">تاريخ التفاعل</label>
              <Input type="date" value={interactionDate} onChange={e => setInteractionDate(e.target.value)} className="h-9 border-border/50 bg-secondary/30" />
            </div>
            <div>
              <label className="mb-1.5 block text-xs text-muted-foreground">موعد المتابعة</label>
              <Input type="date" value={followUpDate} onChange={e => setFollowUpDate(e.target.value)} className="h-9 border-border/50 bg-secondary/30" />
            </div>
          </div>
          <div>
  ...
  return (
    <div className="space-y-5" dir="rtl">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <Button disabled size="sm" className="gap-2 bg-cyan-600/50 hover:bg-cyan-600/50 text-white/50 cursor-not-allowed text-xs">
      ...
      <FeatureUnavailable 
        title="إدارة علاقات العملاء غير متاحة"
        description="ميزة تتبع تفاعلات العملاء والـ CRM غير مدعومة في هذا الإصدار لعدم توفر نقاط النهاية الخاصة بها في خادم الخلفية."
      />
    </div>
  )
}
