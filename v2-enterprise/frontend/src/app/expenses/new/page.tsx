  ...
  return (
    <div className="mx-auto max-w-3xl space-y-5" dir="rtl">
      <div className="flex items-center justify-between gap-4">
        ...
        <Button variant="ghost" onClick={() => router.push('/expenses')} className="gap-2 border border-border/50">
      ...
      <form onSubmit={submit} className="glass rounded-lg p-5 space-y-4">
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          <div className="md:col-span-2">
            ...
            <Input value={form.title} onChange={(e) => setForm((current) => ({ ...current, title: e.target.value }))} required className="border-border/50 bg-secondary/30" />
          </div>
          <div>
          ...
          <div>
            ...
            <Select value={form.currency} onValueChange={(value: 'USD' | 'IQD') => setForm((current) => ({ ...current, currency: value }))}>
              <SelectTrigger className="border-border/50 bg-secondary/30"><SelectValue /></SelectTrigger>
              <SelectContent>
          ...
          <div>
            ...
            <Input value={form.category} onChange={(e) => setForm((current) => ({ ...current, category: e.target.value }))} className="border-border/50 bg-secondary/30" />
          </div>
          <div className="md:col-span-2">
            <label className="mb-1.5 block text-xs text-muted-foreground">ملاحظات</label>
            <Textarea value={form.notes} onChange={(e) => setForm((current) => ({ ...current, notes: e.target.value }))} className="min-h-28 border-border/50 bg-secondary/30" />
          </div>
        </div>
