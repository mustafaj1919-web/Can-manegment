...
function PnLTable({ rows, title, isLocalLight }: { rows: PnLRow[]; title: string; isLocalLight?: boolean }) {
  return (
    <div className={cn(
      ...
      <div className={cn("border-b px-5 py-4 transition-colors", isLocalLight ? "border-slate-200 bg-slate-100/50" : "border-border/50")}>
        <h2 className={cn("text-sm font-bold transition-colors", isLocalLight ? "text-muted-foreground" : "text-foreground")}>{title}</h2>
      </div>
          ...
          {rows.map((row, i) => (
            <tr key={i} className={cn(
              ...
              isLocalLight
                ...
                : cn('border-border/20', row.isTotal ? 'bg-secondary/30' : 'hover:bg-secondary/10')
            )}>
              <td className={cn(
                'py-3 text-xs transition-colors',
                row.isTotal
                  ? cn('px-5 font-bold', isLocalLight ? 'text-muted-foreground' : 'text-foreground')
                  : row.indent
                    ? cn('pr-10 pl-5', isLocalLight ? 'text-muted-foreground' : 'text-muted-foreground')
                    : cn('px-5', isLocalLight ? 'text-muted-foreground' : 'text-foreground/80')
              )}>
                {row.label}
              </td>
              <td className={cn(
                ...
                row.negative || row.value < 0
                  ...
                  : row.isTotal
                    ...
                    : (isLocalLight ? 'text-muted-foreground' : 'text-foreground/90'),
              )}>
    ...
    <div className="space-y-5" dir="rtl">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex gap-2">
          ...
          <Button asChild variant="ghost" size="sm" className="gap-2 border border-border/50 text-xs">
      ...
      <div className="glass rounded-xl p-4">
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-4">
          ...
          <div className="flex items-end">
            <Button variant="ghost" onClick={() => refetch()} disabled={isFetching} className="h-9 w-full border border-border/50 text-xs">
              تحديث البيانات
            </Button>
          </div>
        </div>
      ...
      {/* Tabs */}
      <div className="flex gap-1 rounded-xl border border-border/50 bg-secondary/20 p-1 w-fit">
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
          ...
          <div className={cn(
            ...
            <div className={cn("border-b px-5 py-4 transition-colors", isLocalLight ? "border-slate-200 bg-slate-100/50" : "border-border/50")}>
              <h2 className={cn("text-sm font-bold transition-colors", isLocalLight ? "text-muted-foreground" : "text-foreground")}>ملخص الصندوق</h2>
            </div>
            <table className="w-full text-sm">
              <tbody>
                {[
                  ...
                  <tr key={row.label} className={cn(
                    "border-b last:border-0 hover:bg-secondary/10 transition-colors",
                    isLocalLight ? "border-slate-200 hover:bg-slate-50" : "border-border/20 hover:bg-secondary/10"
                  )}>
                    <td className={cn("px-5 py-3 text-xs transition-colors", isLocalLight ? "text-muted-foreground" : "text-foreground/80")}>{row.label}</td>
