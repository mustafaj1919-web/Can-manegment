...
function NotifItem({ item, level }: { item: Notification; level: keyof typeof LEVEL_MAP }) {
  ...
  return (
    <div className="flex items-start gap-3 px-3 py-2.5 hover:bg-secondary/30 rounded-xl transition-colors">
          ...
          <AnimatePresence>
            {urgentCount > 0 && (
              <motion.span
                ...
                animate={{ scale: 1 }}
                exit={{ scale: 0 }}
                className="absolute -top-0.5 -end-0.5 flex h-4 min-w-4 items-center justify-center rounded-full bg-rose-500 px-0.5 text-[9px] font-bold text-white ring-2 ring-background"
              >
            ...
            {defaultingCount > 0 && (
              <div>
                ...
                {((data as any)?.defaulting_customers as DefaultingCustomer[])?.map(c => (
                  <div key={c.customer_id} className="flex items-start gap-3 px-3 py-2.5 hover:bg-secondary/30 rounded-xl transition-colors">
          ...
          {data?.audit_logs && data.audit_logs.length > 0 && (
            <div style={{ borderTop: '1px solid var(--glass-border)' }} className="p-2">
              ...
              {data.audit_logs.slice(0, 3).map((log) => (
                <div key={log.id} className="flex items-start gap-2.5 px-3 py-2 hover:bg-secondary/20 rounded-xl">
