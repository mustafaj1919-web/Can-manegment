...
function SeverityIcon({ severity }: { severity: string }) {
  if (severity === 'critical') return <ShieldAlert className="h-4 w-4 text-rose-600 dark:text-rose-400 shrink-0"   />
  if (severity === 'high')     return <AlertTriangle className="h-4 w-4 text-orange-600 dark:text-orange-400 shrink-0" />
  if (severity === 'warning')  return <Zap className="h-4 w-4 text-amber-600 dark:text-amber-400 shrink-0"           />
  return <Info className="h-4 w-4 text-cyan-600 dark:text-cyan-400 shrink-0" />
}
