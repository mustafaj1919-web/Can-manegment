...
const META: Record<string, { icon: React.ElementType; color: string }> = {
  car:      { icon: Car,         color: 'text-muted-foreground'   },
  sale:     { icon: TrendingUp,  color: 'text-emerald-400' },
  customer: { icon: Users,       color: 'text-muted-foreground'   },
  purchase: { icon: ShoppingBag, color: 'text-muted-foreground'   },
...
function arabicAction(action: string | null | undefined) {
  if (!action) return '—'
        ...
        <div className="relative">
          ...
          <div className="space-y-0">
            {logs.map((log, i) => {
              ...
              const meta = META[key] ?? { icon: Activity, color:'text-muted-foreground' }
