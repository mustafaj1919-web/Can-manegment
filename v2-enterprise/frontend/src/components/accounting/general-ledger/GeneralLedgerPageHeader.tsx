...
import {
  ...
  ChevronLeft, Building2, Calendar, DollarSign, Clock
} from 'lucide-react'
...
interface GeneralLedgerPageHeaderProps {
  onExportExcel: () => void
  onExportPdf: () => void
  onPrint: () => void
  onRefresh: () => void
...
export function GeneralLedgerPageHeader({
  onExportExcel,
  onExportPdf,
  onPrint,
  onRefresh,
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="space-y-1.5">
          ...
          <nav className="flex items-center gap-1.5 text-xs font-semibold text-[#667085]">
            ...
            <ChevronLeft className="h-3.5 w-3.5 text-[#98A2B3] rotate-180" />
            <span className="text-[#175CD3] font-bold">دفتر الأستاذ العام</span>
          </nav>
          ...
          <div>
            ...
            <p className="text-xs sm:text-sm text-[#475467] mt-0.5">
              استعراض جميع الحركات المحاسبية والرصيد الجاري للحسابات وفقاً للمعايير المحاسبية المعتمدة
            </p>
          </div>
