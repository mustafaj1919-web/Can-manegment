...
function AchievementBar({ pct, color }: { pct: number | null; color: string }) {
  if (pct === null) return <div className="h-1.5 rounded-full bg-white/5 text-[9px] text-center text-muted-foreground/30">—</div>
...
function EmployeeCard({ emp, onSetTarget }: { emp: EmployeePerf; onSetTarget: (emp: EmployeePerf) => void }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
      className={`glass rounded-xl border p-5 ${emp.rank === 1 ? 'border-amber-500/30 ring-1 ring-amber-500/20' : 'border-white/[0.06]'}`}
    >
      {/* Top row */}
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-3">
          <div className={`flex h-10 w-10 items-center justify-center rounded-xl border ${emp.rank <= 3 ? 'border-amber-500/25 bg-amber-500/10' : 'border-white/10 bg-white/5'}`}>
        ...
        <div className="flex items-center gap-2">
          ...
          <Button variant="ghost" size="sm" onClick={() => onSetTarget(emp)} className="h-7 px-2 text-[11px] border border-white/10 gap-1">
      ...
      {/* Bottom row */}
      <div className="mt-3 flex items-center gap-3 border-t border-white/[0.04] pt-3 text-[11px] text-muted-foreground/60">
...
function SetTargetForm({ emp, period, onClose }: { emp: EmployeePerf; period: string; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" onClick={e => e.target === e.currentTarget && onClose()}>
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="glass w-full max-w-sm rounded-xl border border-white/10 p-6">
        ...
        <div className="space-y-3">
          <div>
            ...
            <Input type="number" value={salesCount} onChange={e => setSalesCount(e.target.value)} className="h-9 border-white/10 bg-white/5" />
          </div>
          <div>
            <label className="mb-1.5 block text-xs text-muted-foreground">هدف الإيراد (USD)</label>
            <Input type="number" value={revenue} onChange={e => setRevenue(e.target.value)} className="h-9 border-white/10 bg-white/5" />
          </div>
          <div>
            <label className="mb-1.5 block text-xs text-muted-foreground">هدف الربح (USD)</label>
            <Input type="number" value={profit} onChange={e => setProfit(e.target.value)} className="h-9 border-white/10 bg-white/5" />
          </div>
    ...
    <div className="space-y-5" dir="rtl">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <Select value={period} onValueChange={setPeriod}>
          <SelectTrigger className="w-44 h-9 border-white/10 bg-white/5 text-xs"><SelectValue /></SelectTrigger>
