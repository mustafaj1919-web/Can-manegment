...
function DealCard({ deal, onMove, onDelete }: {
  ...
  return (
    <motion.div layout initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
      className="group glass rounded-xl border border-white/[0.06] p-3.5 hover:border-white/[0.12] transition-all"
    >
      ...
      {deal.notes && (
        <p className="mt-2 text-[11px] text-muted-foreground/70 line-clamp-2 border-t border-white/[0.04] pt-2">{deal.notes}</p>
      )}
...
function AddDealForm({ onClose, onSuccess }: { onClose: () => void; onSuccess: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" onClick={e => e.target === e.currentTarget && onClose()}>
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="glass w-full max-w-md rounded-xl border border-white/10 p-6 shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input type="number" value={expectedPrice} onChange={e => setExpectedPrice(e.target.value)} placeholder="0" className="h-9 w-full rounded-lg border border-white/10 bg-white/5 px-3 text-xs text-foreground focus:outline-none" />
            </div>
            <div>
          ...
          <div>
            ...
            <textarea value={notes} onChange={e => setNotes(e.target.value)} rows={2} className="w-full rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-xs resize-none focus:outline-none" />
          </div>
...
export default function PipelinePage() {
  ...
  const { data, isLoading } = useQuery({ queryKey: ['pipeline'], queryFn: () => getPipeline(), staleTime: 30_000 })

  const moveMut = useMutation({
    mutationFn: ({ id, stage }: { id: number; stage: PipelineStage }) => moveDealStage(id, stage),
    onSuccess: () => { toast.success('تم تحريك الصفقة'); qc.invalidateQueries({ queryKey: ['pipeline'] }) },
    onError: (e) => toast.error(extractApiError(e)),
  })
  ...
  return (
    <div className="space-y-5" dir="rtl">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <Button onClick={() => setShowForm(true)} size="sm" className="gap-2 bg-violet-600 hover:bg-violet-500 text-white text-xs">
      ...
      {data && (
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          <div className="glass rounded-xl p-4 text-center">
            ...
            <p className="font-numeric text-2xl font-black text-foreground mt-1">{data.total_deals}</p>
          </div>
          <div className="glass rounded-xl p-4 text-center">
            <p className="text-xs text-muted-foreground">صفقات نشطة</p>
            <p className="font-numeric text-2xl font-black text-cyan-300 mt-1">{data.active_deals}</p>
          </div>
          <div className="glass rounded-xl p-4 text-center">
            <p className="text-xs text-muted-foreground">معدل الإغلاق</p>
            <p className="font-numeric text-2xl font-black text-emerald-300 mt-1">{data.conversion_rate}%</p>
          </div>
          <div className="glass rounded-xl p-4 text-center">
            <p className="text-xs text-muted-foreground">قيمة متوقعة</p>
            <p className="font-numeric text-sm font-black text-amber-300 mt-1">{money(data.total_expected_iqd)}</p>
          </div>
        </div>
      )}
      ...
      {/* Kanban board */}
      {isLoading ? (
        <div className="flex gap-3 overflow-x-auto pb-2">{Array.from({ length: 5 }).map((_, i) => <Skeleton key={i} className="h-64 w-64 shrink-0 rounded-xl" />)}</div>
      ) : (
        <div className="overflow-x-auto pb-3">
          <div className="flex gap-3 min-w-max">
            {STAGES.map(stage => {
              const deals = data?.by_stage[stage] ?? []
              return (
                <div key={stage} className="w-60 shrink-0">
                  {/* Column header */}
                  <div className={`flex items-center justify-between rounded-t-xl border px-3 py-2.5 ${STAGE_HEADER[stage]}`}>
                    <span className="text-xs font-semibold text-foreground">{STAGE_LABELS[stage]}</span>
                    <span className="flex h-5 min-w-5 items-center justify-center rounded-full bg-white/10 px-1.5 font-numeric text-[10px] font-bold text-foreground">{deals.length}</span>
                  </div>
                  {/* Cards */}
                  <div className="min-h-[200px] rounded-b-xl border border-t-0 border-white/[0.06] bg-white/[0.02] p-2 space-y-2">
                    <AnimatePresence>
                      {deals.map(deal => (
                        <DealCard
                          key={deal.id}
                          deal={deal}
                          onMove={(s) => moveMut.mutate({ id: deal.id, stage: s })}
                          onDelete={() => deleteMut.mutate(deal.id)}
                        />
                      ))}
                    </AnimatePresence>
                    {deals.length === 0 && (
                      <div className="flex items-center justify-center h-20 text-[11px] text-muted-foreground/30">لا توجد صفقات</div>
                    )}
                  </div>
                </div>
              )
      ...
      <AnimatePresence>
        {showForm && <AddDealForm onClose={() => setShowForm(false)} onSuccess={() => { setShowForm(false); qc.invalidateQueries({ queryKey: ['pipeline'] }) }} />}
      </AnimatePresence>
    </div>
  )
}
