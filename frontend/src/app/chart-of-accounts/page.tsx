...
function AccountFormDialog({ mode, initial, allAccounts, onClose, onSuccess }: AccountFormProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <motion.div initial={{ opacity: 0, scale: 0.95, y: 16 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 16 }} className="glass w-full max-w-md rounded-xl border border-white/10 p-6 shadow-2xl">
        <div className="mb-5 flex items-center justify-between">
          <h3 className="text-sm font-bold text-foreground">{mode === 'add' ? 'إضافة حساب جديد' : 'تعديل الحساب'}</h3>
          <button onClick={onClose} className="rounded-lg p-1.5 text-muted-foreground hover:bg-white/10 hover:text-foreground transition-colors"><X className="h-4 w-4" /></button>
        </div>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <Input value={code} onChange={(e) => setCode(e.target.value)} placeholder="111001" required className="h-9 border-white/10 bg-white/5 font-numeric" />
            </div>
            <div>
              <label className="mb-1.5 block text-xs text-muted-foreground">نوع الحساب <span className="text-rose-400">*</span></label>
              <Select value={type} onValueChange={setType} required>
                <SelectTrigger className="h-9 border-white/10 bg-white/5"><SelectValue placeholder="اختر النوع" /></SelectTrigger>
          ...
          <div>
            ...
            <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="اسم الحساب بالعربية" required className="h-9 border-white/10 bg-white/5" />
          </div>
          <div>
            <label className="mb-1.5 block text-xs text-muted-foreground">الحساب الأب</label>
            <Select value={parentId} onValueChange={setParentId}>
              <SelectTrigger className="h-9 border-white/10 bg-white/5"><SelectValue placeholder="بدون أب (حساب رئيسي)" /></SelectTrigger>
              <SelectContent>
          ...
          <div>
            ...
            <Select value={clf} onValueChange={setClf}>
              <SelectTrigger className="h-9 border-white/10 bg-white/5"><SelectValue placeholder="بدون تصنيف" /></SelectTrigger>
              <SelectContent>
...
function TreeRow({ node, depth, search, expanded, onToggle, onEdit, onDelete, onReactivate, onAddChild }: TreeRowProps) {
  ...
  const clfTone = CLF_TONE[node.classification] ?? 'border-white/10 bg-white/5 text-muted-foreground'
    ...
    <>
      ...
      >
        ...
        <td className="px-4 py-2.5 whitespace-nowrap">
          <span className={`inline-flex rounded-full border px-2 py-0.5 text-[10px] ${TYPE_TONE[node.type] ?? 'border-white/10 bg-white/5'}`}>
            {node.type_label}
          </span>
        </td>
    ...
    <div className="space-y-5" dir="rtl">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <Button variant="ghost" size="sm" onClick={() => recomputeMut.mutate()} disabled={recomputeMut.isPending} className="gap-2 border border-white/10 text-xs">
      ...
      <div className="relative">
        ...
        <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="بحث بالرمز أو اسم الحساب..." className="h-10 border-white/10 bg-white/5 pr-9" />
      </div>
        ...
        <>
            ...
            <div className="glass overflow-hidden rounded-lg">
              ...
              <div className="grid grid-cols-2 divide-x divide-x-reverse divide-white/[0.04] sm:grid-cols-3 lg:grid-cols-5">
                {CLF_ORDER.filter(k => data.classification_summary[k]).map(key => {
                  ...
                  const tone  = CLF_TONE[key] ?? 'border-white/10 bg-white/5 text-slate-300'
                  return (
          ...
          {clfOptions.length > 0 && (
            <div className="flex flex-wrap items-center gap-2">
              ...
              <button onClick={() => setActiveClf('')} className={`rounded-full border px-3 py-1 text-[11px] transition-colors ${activeClf === '' ? 'border-primary/50 bg-primary/20 text-primary' : 'border-white/10 bg-white/5 text-muted-foreground hover:border-white/20 hover:text-foreground'}`}>الكل</button>
              {clfOptions.map(key => (
                <button key={key} onClick={() => setActiveClf(activeClf === key ? '' : key)}
                  className={`rounded-full border px-3 py-1 text-[11px] transition-colors ${activeClf === key ? (CLF_TONE[key] ?? 'border-primary/50 bg-primary/20 text-primary') : 'border-white/10 bg-white/5 text-muted-foreground hover:border-white/20 hover:text-foreground'}`}>
          ...
          <div className="glass overflow-hidden rounded-lg">
            <div className="flex items-center justify-between border-b border-white/[0.06] px-5 py-3.5">
              ...
              <div className="flex items-center gap-1.5">
                <button onClick={expandAll}   className="rounded-lg border border-white/10 px-2.5 py-1 text-[11px] text-muted-foreground hover:text-foreground transition-colors">توسيع الكل</button>
                <button onClick={collapseAll} className="rounded-lg border border-white/10 px-2.5 py-1 text-[11px] text-muted-foreground hover:text-foreground transition-colors">طي الكل</button>
              </div>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full min-w-[1120px] text-sm">
                <thead>
                  <tr className="border-b border-white/[0.06] bg-white/[0.03] text-xs text-muted-foreground">
