  ...
  return (
    <div className="space-y-6" dir="rtl">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-white/5 pb-4">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-xs text-neutral-400 mt-0.5">اكتشف أحدث السيارات المميزة والمعروضة للبيع</p>
          </div>
        </div>

        <div className="flex p-0.5 rounded-lg bg-secondary/50 border border-white/5">
          <button
      ...
      {!isLoading && featuredHeroCar && (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 rounded-2xl border border-white/10 overflow-hidden relative"
          style={{
        ...
        >
          ...
          <div className="lg:col-span-6 p-6 sm:p-8 flex flex-col justify-between z-10 space-y-6">
            <div className="space-y-4">
              <div className="flex items-center gap-2">
                <span className={cn(
                  'rounded px-2.5 py-0.5 text-[9px] font-black border border-white/5 uppercase tracking-wider',
                  getStatusVariant(featuredHeroCar.status)
                )}>
                  {translateStatus(featuredHeroCar.status)}
                </span>
                <span className="text-[10px] text-neutral-400 font-bold border border-white/5 px-2 py-0.5 rounded bg-white/5">
              ...
              <div>
                ...
                <p className="text-xs text-neutral-400 mt-1">{featuredHeroCar.trim || 'فئة قياسية'}</p>
              </div>
              ...
              <div className="grid grid-cols-3 gap-3 pt-2">
                <div className="bg-white/[0.02] border border-white/5 p-2 rounded-lg text-center">
                  ...
                  <span className="text-[10px] text-neutral-400 block">الوقود</span>
                ...
                <div className="bg-white/[0.02] border border-white/5 p-2 rounded-lg text-center">
                  ...
                  <span className="text-[10px] text-neutral-400 block">الناقل</span>
                  <span className="text-xs font-bold text-white block mt-0.5">
                ...
                <div className="bg-white/[0.02] border border-white/5 p-2 rounded-lg text-center">
                  ...
                  <span className="text-[10px] text-neutral-400 block">العداد</span>
                  <span className="text-xs font-bold text-white block mt-0.5">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pt-4 border-t border-white/5">
              <div>
                <span className="text-[10px] text-neutral-500 block">سعر البيع</span>
              ...
              <div className="flex items-center gap-2">
                ...
                <Button asChild variant="outline" size="sm" className="border-white/10 hover:bg-white/5 text-white text-xs h-9">
          ...
          <div className="lg:col-span-6 min-h-[260px] bg-red-950/10 flex items-center justify-center p-6 relative overflow-hidden">
            <div className="absolute top-2 left-2 z-20 flex items-center gap-1">
              <button onClick={handlePrev} className="h-8 w-8 rounded-full bg-black/60 border border-white/10 flex items-center justify-center text-white hover:bg-red-600 transition-all">
                <ChevronRight className="h-4 w-4" />
              </button>
              <button onClick={handleNext} className="h-8 w-8 rounded-full bg-black/60 border border-white/10 flex items-center justify-center text-white hover:bg-red-600 transition-all">
        ...
        {isLoading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {Array.from({ length: 4 }).map((_, i) => (
              <div key={i} className="bg-secondary/20 border border-white/5 rounded-xl p-3 space-y-3">
          ...
          <div className="border border-dashed border-white/10 rounded-xl py-12 text-center">
            ...
            <p className="text-sm text-neutral-400 font-family-cairo">لا تتوفر سيارات لعرضها حالياً</p>
                ...
                <div key={car.id} className="group bg-[#111111] border border-white/5 rounded-xl overflow-hidden hover:border-red-500/30 transition-all duration-300 flex flex-col justify-between">
                  ...
                  <div className="p-3 space-y-2 flex-1 flex flex-col justify-between">
                    <div>
                      ...
                      <p className="text-[10px] text-neutral-400 mt-0.5">{car.manufacturing_year} · {car.color || 'غير محدد'}</p>
                    </div>

                    <div className="border-t border-white/5 pt-2 flex items-center justify-between mt-2">
                      <span className="text-xs font-black text-red-500 font-numeric">
                        {car.selling_price ? formatMoney(car.selling_price, car.currency) : '—'}
                      </span>
                      <Button asChild size="icon-sm" variant="ghost" className="h-6 w-6 rounded bg-white/5 text-neutral-300 hover:bg-red-600 hover:text-white transition-all">
